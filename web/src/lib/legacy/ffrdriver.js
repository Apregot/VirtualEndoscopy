...
export class FFR__driver {
   ...
   */	
    ...
    async start (
        ...
        const ret_cube = await this._ffrsrv.TransferCube(this._stack, msg => {
        ...
        if (!resp.result) { FFR.viewer.showInfoMessage(resp.reason); } else {
            ...
            d8.exec(
                e => {
                    ...
                    const x = circle.center.x.toFixed(2); const y = circle.center.y.toFixed(2); const z = circle.center.z.toFixed(2);
	 ...
	 * @param {!number=} radius
     *
     * upd: уже после того, как выбрали нужную аорту и прожали accept
	 */
    async initializeAndSegmentAorta (center, radius) {
        const x = center.x.toFixed(2); const y = center.y.toFixed(2); const z = center.z.toFixed(2); const r = radius.toFixed(2);
  	...
  	// const FFRSettings = {};
        ...
        const ret = await this._ffrsrv.Initialize(x, y, z, r);		// ответ только ok
        ...
        const optimal_tau = await this._ffrsrv.GetOptimalTau();
        if (optimal_tau > 0.0) { this._aortaPreviewState.tau = optimal_tau; }
    ...
    async aortaPreview () {
        ...
        const d10 = new Dialog10(this._ffrsrv, this._aortaPreviewState, this.visArea);
        d10.exec(e => {
            if (e == 'ok') {
    ...
    async step3 () {
        const tau = this._aortaPreviewState.tau; const alpha = this._aortaPreviewState.alpha;
        const beta = this._aortaPreviewState.beta; const gamma = this._aortaPreviewState.gamma;
  	...
  	console.log('SetVesselsSeeds returns', performance.now());
        ...
        t0 = performance.now();
        const aortaObj = await this._ffrsrv.LoadAortaObject();
        console.log('LoadAortaObject returns in', ((performance.now() - t0) / 1000).toFixed(3), 'сек');
        const obj = FSF.parseBuffer(aortaObj); // console.log('obj',obj);
        const aortaCube = new Cube(obj[0], 'aorta');
        const aortaGeometry = aortaCube.geometry();
        const aortaMesh = new THREE.Mesh(aortaGeometry, new THREE.MeshLambertMaterial({ color: 0x00fafa, side: THREE.DoubleSide }));

        // Вставляем в сцену новую аорту, а старую удаляем
        const scene = this.visArea.scene;
        scene.add(aortaMesh);
        scene.remove(this.aortaPreviewMesh);
        this.aortaPreviewMesh = aortaMesh;

  	const d11 = new Dialog11(this._ffrsrv, this.visArea);
  	d11.exec(e => {
  		console.log(e);
  		if (e == 'cancel') {
  			this.aortaPreview();
    ...
    loadBlobAsUint8Array (file) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            reader.onload = function () {
                ...
                const mask = new Uint8Array(/** @type {ArrayBuffer} */ (reader.result));
                resolve(mask);
            };
    ...
    async step4 () {
  	...
  	let t0 = performance.now();
  	const retcode = await this._ffrsrv.Thinning(this._aortaPreviewState, this._vesselsThreshold, e => this.showProgress(e));
  	...
  	console.log('LoadAortaObject returns in', ((performance.now() - t0) / 1000).toFixed(3), 'сек');
        ...
        const vesselObjectsCount = await this._ffrsrv.GetVesselsObjectsCount();
        ...
        const vesselCube = [];
        for (let i = 0; i < vesselObjectsCount; i++) {
            const vesselData = await this._ffrsrv.LoadVesselsObject(i);
            const obj = FSF.parseBuffer(vesselData);
            vesselCube.push(new Cube(obj[0], 'vessel' + i));
        }
        // console.log('vesselCube',vesselCube)

        const visScene = this.visArea.scene;
	  // убираем объекты со сцены
        for (let i = 0; i < visScene.children.length; i++) {
            if (visScene.children[i].type == 'Mesh') { visScene.remove(visScene.children[i]); } 
        }
        // console.log('this.visArea.scene',this.visArea.scene);

  	console.log('---------------------step 4 finished----------------- ');
  	const v = await this.getVesselInfo();

        console.log('calling processMarkStenosis');
        const state4Obj = { stack: this._stack, aortaCube, vessels: vesselCube, vesselInfo: v };
    ...
    async getVesselInfo () {
        ...
        const vesselsCount = await this._ffrsrv.GetVesselsCount();
        ...
        for (let i = 0; i < vesselsCount; i++) {
            ...
            const parts = info.split(' '); parts.splice(0, 2);
            info = parts.join(' ');
    ...
    showProgress (msg) {
        ...
        if (part.length > 1) {
            ...
            if (what == 'fin') {
                this.hideProgress();
            } else { self._progressBar.setMessage(what); }
        } else { console.log('showProgress(): неверный формат: ', msg); }
    }
    ...
    async WS_test () {
        ...
        const res = await this._ffrsrv.sendRequestFSF('test stream cube');
        console.log('res', res);
    }
}	// class FFR__driver
...
function nonzero_stats (blob) {
    return new Promise(function (resolve, reject) {
        ...
        fileReader.onload = function (event) {
            ...
            const view = new Uint8Array(/** @type {ArrayBuffer} */(arrayBuffer));	
            const non0 = { total: view.length, nonzero: 0, first: -1, last: -1 };
...
function blobAsFloat64Array (blob) {
    return new Promise(function (resolve, reject) {
        ...
        fileReader.onload = function (event) {
            const view = new Float64Array(/** @type {ArrayBuffer} */(fileReader.result));	
            resolve(view);
        };	
...
function blobAsUint8Array (blob) {
    return new Promise(function (resolve, reject) {
        ...
        fileReader.onload = function (event) {
            const view = new Uint8Array(/** @type {ArrayBuffer} */(fileReader.result));	
            resolve(view);
        };	
...
function Uint8ArrayToHex (arr) {
    let str = '';
    const len = (arr.length < 16 ? arr.length : 10);
    for (let i = 0; i < len; i++) { str += ' 0x' + arr[i].toString(16); }
    if (len < arr.length) { str += '...'; }
    return str;
}
...
function fromUTF8Array (data, offset, length) { // array of bytes
    let str = '';
    const i0 = (offset || 0); const maxInd = i0 + (length || data.length);
...
function addSphereAtPoint (scene, point) {
    ...
    const faceColorMaterial = new THREE.MeshBasicMaterial({ color: 0x0000ff });
...
function seriesResize (series, bounds) {
    	...
    	const dims = series.stack[0].dimensionsIJK; const limits = [dims.x, dims.y, dims.z];
    ...
    const newSeries = new AMI.SeriesModel();
    for (const key in series) { newSeries[key] = series[key]; }

    // создаем копию стека. для массива frame[] создаем собственную копию
    const oldStack = series.stack[0]; const newStack = new AMI.StackModel();
    for (const key in oldStack) { newStack[key] = oldStack[key]; }
    newStack.frame = [];
...
function frameResize (srcFrame, boundsX, boundsY) {
    ...
    const frame = Object.assign(new AMI.FrameModel(), srcFrame);
    ...
    const pixelData = new Int16Array(columns * rows);
    let i = 0;

    for (let y = boundsY[0]; y <= boundsY[1]; y++) {
        for (let x = boundsX[0]; x <= boundsX[1]; x++) { pixelData[i++] = srcFrame.pixelData[y * srcFrame.columns + x]; } 
    }
