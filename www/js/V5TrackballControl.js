 ...
 * @author Nicolas Rannou / https://github.com/FNNDSC/ami
 */
/**
 * За остнову класса V5TrackballControl взят AMI.TrackballOrthoControl, который является урезанной
 * версией AMI.TrackballControl, который в свою очередь является модифицированной версией THREE.TrackballControl
 * В V5TrackballControl возвращена назад функциональность rotate и custom, соотвественно появился STATE CUSTOM
 * возвращено явное управление состоянием через setState() и убрано неявное включение состояния в зависимости
 * от нажатой кнопки мыши. Надо бы еще убрать включение состояния по нажатию клавиш ASD на клавиатуре
 */
'use strict';
const STATE = {NONE: -1, ROTATE: 1, ZOOM: 2, PAN: 0, SCROLL: 4, TOUCH_ROTATE: 4, TOUCH_ZOOM_PAN: 5, CUSTOM: 99};
/**
 * @extends {THREE.EventDispatcher}
 */
class V5TrackballControl extends THREE.EventDispatcher {
    constructor(object, domElement) {
      ...
      let getMouseOnCircle = (function() {
        ...
        // ROTATE листает фреймы
      this.rotateCamera = function() {
        let factor;
          _this.cnt++;
          let delta = _moveCurr.y - _movePrev.y;
          if(delta === 0) return;
      ...
      function mousedown(event) {
        ...
        if(!isMyState()) return;
        //console.log('V5TrackbalControl::mousedown()');
  
        _state = _this.forceState;
